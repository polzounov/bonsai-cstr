...
for idx in range(0, sim_max):
    ...
    m.time = np.linspace(
        0, 45, num=90
    )  # simulate 45 minutes (0.5 minute per controller action)
    ...
    def cstr(x, t, u, Tf, Caf):
        ...
        dTdt = q / V * (Tf - T) - mdelH / (rhoCp) * rA + ((UA / (V * rhoCp)) * (Tc - T))
    ...
    T_ = interpolate.interp1d(
        [0, p1, p2, time, time + 1], [311.2612, 311.2612, 373.1311, 373.1311, 373.1311]
    )
