...
class CSTRSimulation:
    def reset(
        ...
        if self.constraint == "ml":
            self.ml_model = loaded_model = pickle.load(
                open("ml_predict_temperature.pkl", "rb")
            )
    ...
    def step(self, k):
        ...
        sigma_max_Ca = error_var * (8.5698 - 2)
        sigma_max_T = error_var * (373.1311 - 311.2612)

        sigma_Ca = random.uniform(-sigma_max1, sigma_max1)
        sigma_T = random.uniform(-sigma_max_T, sigma_max_T)
        mu = 0

        y = 0
        if self.constraint == "ml" and self.T >= 340:
            X = [[self.Ca, self.T, self.Tc, self.dTc]]
            y = self.ml_model.predict(X)[0]
            if self.ml_model.predict_proba(X)[0][1] >= 0.3:
                y = 1

        # Import MPC
        MPC_Tc = non_linear_mpc_model.non_lin_mpc(
            0, CrSP, self.Ca, self.T, self.Tc + self.dTc
        )
        ...
        model = cstr.CSTRModel(T=self.T, Ca=self.Ca, Tc=self.Tc, dTc=self.dTc)

        self.Tc += self.dTc
        self.T = model.T + sigma_T
        self.Ca = model.Ca + sigma_Ca

        # Constraints - ML model to restrict action when a Thermal Runaway scenario is detected for next state
        if self.constraint == "ml":
            if y == 1:
                new_dTc = self.dTc
                model2 = cstr.CSTRModel(T=self.T, Ca=self.Ca, Tc=self.Tc, dTc=new_dTc)
                while new_dTc <= 10 and new_dTc >= -10:
                    # reduce in 5%
                    new_dTc -= 0.05 * abs(self.dTc) * np.sign(self.dTc)
                    X = [[self.Ca, self.T, self.Tc, new_dTc]]
                    y = self.ml_model.predict(X)[0]
                    if self.ml_model.predict_proba(X)[0][1] >= 0.3:
                        y = 1
                    if y == 0:
    ...
    def episode_step(self, dTc, k) -> None:
    ...
    def halted(self) -> bool:
        if self.T >= 400:
            ...
            return True
        elif self.dTc > 10 or self.dTc < -10:
...
def main():
    ...
    for k in range(time):
        ...
        df_t = pd.DataFrame(
            [[cstr_sim.Ca, cstr_sim.T, cstr_sim.Tc]], columns=["Ca0", "T0", "Tc0"]
        )
        ...
        cstr_sim.episode_step(cstr_sim.dTc, k)
        state = cstr_sim.get_state()
